...
#进行kmean聚类划分
def kmean(train,k):
    dataNums=train.shape[0]
    #用InitialCluster获得初始的聚类中心值
    InitialCluster =[]
    for i in range(k):
        InitialCluster.append(train[i])
    #将输入数据进行复制
    Data = tile(train, (k, 1))
    while True: 
        errData=(Data-array(InitialCluster).reshape(10,1))**2
        
        Idx = classfic(errData, dataNums)
        #updateCluster为更新后所得到的聚类中心
        updateCluster=[]
        #对于每一类进行循环求得分类的中心
        for i in range(k):
            Sum=0;Count=0
            for j in range(dataNums):  
                if Idx[j]==i:
                    #用Sum进行求和处理，Count对每一类进行计数处理
                    Sum=Sum+train[j]
                    Count=Count+1
            #求得更新后的均值
            Cluster=Sum/Count
            updateCluster.append(Cluster)
        #如果更新后聚类中心与更新前聚类中心相同则代表获得最终的聚类中心值
        if(updateCluster==InitialCluster):
            break
        InitialCluster=updateCluster
    return Idx,updateCluster


#获得径向基扩展常数
def variance(datanums,index,means,m,train):
    Var=[];MAX=0
    for i in range(m):
        sum=0;Count=0
        for j in range(datanums):    
            if index[j]==i:
                Count=Count+1
                if MAX<((train[j]-means[i])**2):
                    MAX=((train[j]-means[i]))
        Getmax=MAX/((2*Count)**0.5)
        Var.append(Getmax)
    return Var


#求得高斯核函数
def Gauss(means,Var,train,m,Excon):
    Data = tile(train, (m, 1))
    Data=Data.T
    for i in range(m):
        for j in range(train.shape[0]):
            Data[j,i]=exp( -(Data[j,i]-means[i])*(Data[j,i]-means[i]) / (2*(Excon[i])**2))
    return Data
...
def main():
    ...
    M=10
    test_x=array(test_x)
    dataNums=test_x.shape[0]
    #index代表数据划分聚类，means为是个聚类中心值大小
    index,means=kmean(test_x,M)
    ...
    Excon=[];
    for i in range(M):
        Excon.append(min(distance[:,i]))
    
    #Var为获得的高斯方差
    Var=variance(dataNums,index,means,M,train_x)
    print("%d个聚类均值: %s" %(M,means))
    print("%d个聚类方差：%s" %(M,Var))


    #通过高斯径向基函数的输出
    Data=Gauss(means,Var,train_x,M,Excon) #经过高斯激活函数后的值 
    b=ones((100,1))#b值
    ...
    testData=Gauss(means,Var,test_x,M,Excon) 
